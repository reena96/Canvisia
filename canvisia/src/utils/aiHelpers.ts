...
export async function executeChangeColor(
  ...
  for (const shape of targetShapes) {
    ...
    if (shape.type === 'text') {
      ...
      await updateShape(canvasPath, shape.id, { fill: input.newColor })
    } else if (shape.type === 'line' || shape.type === 'arrow' || shape.type === 'bidirectionalArrow') {
      // Lines/arrows use 'stroke' for color
      await updateShape(canvasPath, shape.id, { stroke: input.newColor })
    } else {
...
export async function executeDeleteElements(
  ...
  for (const shape of targetShapes) {
    await deleteShape(canvasPath, shape.id)
  }
...
export async function executeCreateFlowchart(
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    await createShape(canvasPath, nodeShape)
  ...
  for (const textLabel of textLabels) {
    await createShape(canvasPath, textLabel)
  }
...
export async function executeCreateDiagram(
  ...
  switch (diagramType) {
    case 'orgchart': {
      ...
      await createShape(canvasPath, ceo)

      await createShape(canvasPath, {
        id: ceoTextId,
        type: 'text',
      ...
      for (let i = 0; i < managerCount; i++) {
        ...
        await createShape(canvasPath, manager)
...
export async function executeUndo(
  ...
  console.log('[AI Helpers] executeUndo called for canvas:', canvasPath)
  ...
  try {
      ...
      case 'create': {
        ...
        if (lastAction.createdShapeIds && lastAction.createdShapeIds.length > 0) {
          for (const shapeId of lastAction.createdShapeIds) {
            await deleteShape(canvasPath, shapeId)
          }
      ...
      case 'modify': {
        ...
        if (lastAction.modifiedShapes && lastAction.modifiedShapes.length > 0) {
          for (const { id, originalState } of lastAction.modifiedShapes) {
            await updateShape(canvasPath, id, originalState)
          }
    ...
    await deleteUndoAction(canvasPath, lastAction.id)
